...
7. **Unfreeze after bet-focus** — thaw play from `bet_focus/` checkpoints (`--unfreeze`, 200k, teacher CE retained); mid-run checkpoints + SIGTERM pause + `scripts/run_pg_unfreeze_train.py --resume`. Bake-off via `scripts/run_pg_spread_bakeoff.py --unfreeze` (not yet a published paper section).
