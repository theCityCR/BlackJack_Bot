...
6. ~~Bet+play policy gradients~~ — REINFORCE / A2C / PPO learn discrete stake + play (`agents/reinforce.py`, `a2c.py`, `ppo.py`); warm-start from SpreadRule; eval via `scripts/run_variable_betting_eval.py --pg-agent …`. §5.7 bake-off published. ~~Bet-focus follow-on~~ — freeze rule play, teacher CE, 500k budget (`--bet-focus`); paired bake-off published as [§5.8](docs/paper.md) (`docs/results/pg_bet_focus_bakeoff_results.json`).
7. ~~Unfreeze after bet-focus~~ — thaw play from `bet_focus/` checkpoints (`--unfreeze`, 200k, teacher CE retained); mid-run checkpoints + SIGTERM pause + `scripts/run_pg_unfreeze_train.py --resume`. Paired bake-off published as [§5.9](docs/paper.md) (`docs/results/pg_unfreeze_bakeoff_results.json`): all three agents go negative vs flat and trail Hi-Lo.
